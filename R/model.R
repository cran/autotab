...
model_VAE = function(data, encoder_info, decoder_info,Lip_en, pi_enc=1,lip_dec, pi_dec=1,latent_dim, feat_dist, lr, beta,max_std=10.0,min_val, temperature=0.5,weighted=0, recon_weights, seperate = 0,prior="single_gaussian",K =3,learnable_mog=FALSE,mog_means=NULL, mog_log_vars=NULL, mog_weights=NULL ){
  ...
  if (seperate == 1) {
    ...
    if (has_cont) {
      vae$add_metric(reconstruction_loss[[metric_idx]], name = "cont_loss", aggregation = "mean")
      metric_idx <- metric_idx + 1
    }
    if (has_bin) {
      vae$add_metric(reconstruction_loss[[metric_idx]], name = "bin_loss", aggregation = "mean")
      metric_idx <- metric_idx + 1
    }
    if (has_cat) {
      vae$add_metric(reconstruction_loss[[metric_idx]], name = "cat_loss", aggregation = "mean")
    }
  }
